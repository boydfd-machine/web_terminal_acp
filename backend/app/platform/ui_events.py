...
from app.platform.polling_response_cache import (
    expire_polling_response_cache_async,
    invalidate_polling_response_cache_async,
)
from app.contexts.activity.application.agent_work_status_cache import clear_agent_work_status_cache
from app.contexts.activity.application.window_activity import clear_client_windows_activity_cache_async

UiEventSender = Callable[[str], Awaitable[None]]
UiResource = str
UI_EVENT_SEND_TIMEOUT_SECONDS = 1.0
...
class UiEventHub:
    ...
    async def publish_invalidation(
        ...
        *,
        ...
        if reason in _EXPIRE_ONLY_REASONS:
            await expire_polling_response_cache_async(normalized_resources, client_id=client_id)
            if {"window", "terminal_notifications"} & set(normalized_resources):
                clear_agent_work_status_cache(client_id, window_id)
        else:
            await invalidate_polling_response_cache_async(normalized_resources, client_id=client_id)
            if {"tree", "window"} & set(normalized_resources):
                await clear_client_windows_activity_cache_async(client_id)
                clear_agent_work_status_cache(client_id, window_id)
        await self._publish(
            {
                "type": "invalidate",
    ...
    async def _publish(self, payload: dict[str, object]) -> None:
        ...
        async def send_to_subscriber(sender: UiEventSender) -> UiEventSender | None:
            try:
                await asyncio.wait_for(sender(message), timeout=UI_EVENT_SEND_TIMEOUT_SECONDS)
            except Exception:
                return sender
            return None
        ...
        failed_senders = [
            sender
            for sender in await asyncio.gather(*(send_to_subscriber(sender) for sender in subscribers))
