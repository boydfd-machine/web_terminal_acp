...
from app.platform.ingest.normalizers import NormalizedEvent, normalize_codex_trace
from app.platform.plugins.agent_tools.adapters.codex_subagents import (
    message_content as codex_message_content,
    message_role,
    subagent_chat_projection,
    subagent_projection,
    subagent_state_parts,
)
...
class CodexAdapter:
    ...
    def normalize(
        ...
        return agent_tool_record_event(
            self.provider_id,
            normalize_codex_trace(payload, source_path=source_path),
        )
    ...
    def project_event(self, event: Event) -> AgentEventProjection:
        ...
        if raw_type == "response_item" and item_type == "message":
            content_body = _content_text(codex_message_content(item))
            body = content_body or json_markdown(item)
            body_format = "markdown" if content_body else "json"
            if role == "user":
                sidechain_projection = subagent_projection(
                    item,
                    body=body,
                    body_format=body_format,
                    event_kind=event.kind,
                    message_type="subagent_call",
                )
                if sidechain_projection is not None:
                    return sidechain_projection
    ...
    def project_chat(self, event: Event) -> AgentChatProjection | None:
        ...
        if raw_type == "response_item" and item_type == "message":
            role = message_role(item)
            body = _content_text(codex_message_content(item))
            if role == "user" and body:
                subagent_chat = subagent_chat_projection(
                    event,
                    item,
                    body=body,
                    message_type="subagent_call",
    ...
    def subagent_state(self, event: Event) -> AgentSubagentState:
        payload = event.payload_json
        item = _item(payload)
        is_sidechain, is_call, is_result = subagent_state_parts(item)
