...
from app.client_agent.agent_idle.antigravity_sessions import (
    antigravity_session_id_from_payload,
    latest_antigravity_session_data,
)
...
class AgentIdleSupervisor:
    ...
    async def _maybe_suspend_provider(
        ...
        if state.session is None or state.last_output_at is None:
            ...
            session = latest_session_ref(window_id, provider, project_path=project_path)
            if session is None:
                return
            state.session = session
            state.last_output_at = session.last_output_at
...
def latest_session_ref(
    ...
    *,
    ...
    if provider == "antigravity_cli":
        return latest_antigravity_session_ref(window_id, project_path=project_path)
    return None
...
def latest_resume_command(window_id: UUID, *, project_path: str | None = None) -> str | None:
    candidates = [
        ...
        if (session := latest_session_ref(window_id, provider, project_path=project_path)) is not None
    ]
    if not candidates:
        return None
...
def latest_antigravity_session_ref(
    window_id: UUID,
    *,
    project_path: str | None = None,
) -> AgentSessionRef | None:
    data = latest_antigravity_session_data(window_id, project_path=project_path)
    if data is None:
        return None
    return AgentSessionRef(
        ...
        last_output_at=data.last_output_at,
    )
...
def session_id_from_payload(
    ...
    if provider == "codex":
        return codex_session_id_from_payload(payload, source_path)
    if provider == "cursor_cli":
        return (
    ...
    if provider == "antigravity_cli":
        return antigravity_session_id_from_payload(payload, source_path)
    return None
