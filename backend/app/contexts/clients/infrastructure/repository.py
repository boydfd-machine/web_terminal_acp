...
async def get_client_for_owner(
    session: AsyncSession, client_id: UUID, owner_user_id: str | None
) -> Client | None:
    if owner_user_id is None:
        return await get_client(session, client_id)
    return await session.scalar(
        select(Client).where(Client.id == client_id, Client.owner_user_id == owner_user_id)
    )
...
def _require_matching_owner(client: Client, owner_user_id: str | None) -> None:
    if client.owner_user_id != owner_user_id:
        raise ClientNameUnavailable("client name is already used by another user")
...
async def create_or_rotate_remote_client_by_name(
    ...
    *,
    ...
    if client is not None:
        _require_matching_owner(client, owner_user_id)
...
async def list_clients_for_owner(session: AsyncSession, owner_user_id: str | None) -> list[Client]:
    if owner_user_id is None:
        return await list_clients(session)
    return list(
        await session.scalars(
            select(Client)
            .where(Client.owner_user_id == owner_user_id)
            .order_by(Client.name, Client.id)
        )
    )


async def claim_unowned_clients(session: AsyncSession, owner_user_id: str) -> None:
    await session.execute(
        sa_update(Client)
        .where(Client.owner_user_id.is_(None))
        .values(owner_user_id=owner_user_id)
    )
    await session.execute(
        sa_update(ClientRegistrationKey)
        .where(ClientRegistrationKey.owner_user_id.is_(None))
        .values(owner_user_id=owner_user_id)
    )
    await session.flush()
...
async def delete_remote_client(session: AsyncSession, client_id: UUID) -> bool | None:
    client = await get_client(session, client_id)
    return await _delete_remote_client(session, client, client_id)


async def delete_remote_client_for_owner(
    session: AsyncSession,
    client_id: UUID,
    owner_user_id: str | None,
) -> bool | None:
    client = await get_client_for_owner(session, client_id, owner_user_id)
    return await _delete_remote_client(session, client, client_id)


async def _delete_remote_client(
    session: AsyncSession,
    client: Client | None,
    client_id: UUID,
) -> bool | None:
    ...
    await session.execute(sa_delete(ProjectTodo).where(ProjectTodo.client_id == client_id))
