...
from app.contexts.terminal_runtime.infrastructure.tmux_manager import TmuxAttachTarget, build_attach_command
...
def parse_text_input(text: str) -> bytes | ResizeControl | SelectWindowControl | OutputAckControl | None:
    ...
    if payload.get("type") == "select_window":
        ...
        if not isinstance(window_id, str):
            return None
        try:
            return SelectWindowControl(
                window_id=UUID(window_id),
                allow_missing_window_recreate=payload.get("allow_missing_window_recreate") is True,
            )
        except ValueError:
            return None
