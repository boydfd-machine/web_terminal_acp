...
async def get_or_create_ai_session(
    ...
    *,
    ...
    ai_session = await _select_ai_session(
        session,
        client_id,
        provider,
        source_id,
    ...
    try:
        ...
        existing_session = await _select_ai_session(
            session,
            client_id,
            provider,
            source_id,
...
async def _select_ai_session(
    ...
    if virtual_window_id is None:
        query = query.where(AiSession.virtual_window_id.is_(None))
    else:
        query = query.where(AiSession.virtual_window_id == virtual_window_id)
