...
from app.contexts.activity.application.terminal_recents import (
...
async def get_terminal_recents(
    ...
    todo_titles = await assigned_todo_titles_by_window(
        session,
        [item.window_id for item, _title in items],
        client_id=client_id,
    )
    return TerminalRecentPageOut(
        items=[
            TerminalRecentOut(
                ...
                title=title,
                todo_title=todo_titles.get(item.window_id),
...
async def get_global_terminal_recents(
    ...
    todo_titles = await assigned_todo_titles_by_window(
        session,
        [item.window_id for item, _title, _client_name in items],
    )
