...
_CLAUDE_TOKEN_USAGE_METRIC_NAME = "claude_code.token.usage"
...
def event_is_agent_failure(event: Event) -> bool:
    payload = event.payload_json
    if codex_failure_payload(payload) or generic_failure_payload(payload, event.kind):
        return True
    return False
...
def event_is_agent_activity(event: Event) -> bool:
    ...
    if event_is_agent_user_input(event) or event_is_agent_completion(event) or event_is_agent_failure(event):
        return True
...
def claude_work_activity_payload(payload: dict) -> bool:
    ...
    if (
        payload_type == "otel_metric"
        and payload_text(payload.get("name")) == _CLAUDE_TOKEN_USAGE_METRIC_NAME
    ):
        return False
...
def cursor_work_activity_payload(payload: dict) -> bool:
    if payload.get("type") == "statusline_token_usage":
        return False
    role = _payload_role(payload)
    if role == "system":
        return False
...
def codex_failure_payload(payload: dict) -> bool:
    raw_type = (
        payload_text(payload.get("raw_type"))
        or payload_text(payload.get("name"))
        or payload_text(payload.get("type"))
    ...
    if not isinstance(item, dict):
        item = payload
    item_type = payload_text(item.get("type"))
    if raw_type in {"error", "task_error", "turn_failed"}:
        return True
    if raw_type == "event_msg" and item_type in {"error", "task_error", "turn_failed"}:
        return True
    return _payload_has_fatal_agent_error_text(payload)
...
def generic_failure_payload(payload: dict, event_kind: str | None = None) -> bool:
    failure_kind = payload_text(event_kind) or payload_text(payload.get("kind"))
    payload_type = payload_text(payload.get("type"))
    subtype = payload_text(payload.get("subtype"))
    error_typed = any(_looks_like_agent_failure_type(value) for value in (failure_kind, payload_type, subtype))
    if error_typed:
        return True
    return _payload_has_fatal_agent_error_text(payload)
...
def _payload_has_fatal_agent_error_text(value: object) -> bool:
    for text in _payload_strings(value):
...
def _looks_like_agent_failure_type(value: str | None) -> bool:
    if value is None:
        return False
