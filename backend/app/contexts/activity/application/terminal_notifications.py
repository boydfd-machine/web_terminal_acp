...
async def clear_terminal_notifications(session: AsyncSession, client_id: UUID) -> None:
    ...
    completed_by_window = {
        notification.window_id: _aware_utc(notification.completed_at)
    ...
    states = {
        ...
        for state in await session.scalars(
            select(TerminalNotificationState).where(
                TerminalNotificationState.client_id == client_id,
                TerminalNotificationState.window_id.in_(tuple(completed_by_window)),
            )
        )
    }
    for notification in notifications:
        state = states.get(notification.window_id)
        if state is None:
            state = TerminalNotificationState(client_id=client_id, window_id=notification.window_id)
            states[notification.window_id] = state
            session.add(state)
        completed = completed_by_window[notification.window_id]
        if not _at_or_after(state.read_at, completed):
            state.read_at = completed
        if not _at_or_after(state.dismissed_at, completed):
            state.dismissed_at = completed
