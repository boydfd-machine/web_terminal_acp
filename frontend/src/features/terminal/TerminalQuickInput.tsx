...
export function TerminalQuickInput({
  ...
  return (
    ...
    >
      <textarea
        ref={textareaRef}
        aria-label={t("terminal.quickInput.aria")}
        value={localValue}
        placeholder={placeholder ?? t("terminal.quickInput.placeholder")}
        spellCheck={false}
        rows={1}
        onChange={(event) => {
      ...
      <div className="terminal-quick-input-actions">
        {canUseQuickKeys && (
          ...
          >
            {t("terminal.quickInput.quickKeys")}
          </button>
        )}
        {onCancel !== undefined && (
        ...
        <button type="submit" disabled={!canSubmit}>
          {submitLabel ?? t("terminal.quickInput.submit")}
        </button>
      </div>
      {quickKeysOpen && canUseQuickKeys && (
        <div className="terminal-quick-key-drawer">
          <input
            type="search"
            aria-label={t("terminal.quickInput.searchQuickKeys")}
            value={quickKeyQuery}
            placeholder={t("terminal.quickInput.searchQuickKeys")}
          ...
          <div className="terminal-quick-key-list" role="list">
            {visibleQuickKeys.length === 0 ? (
              <p className="muted terminal-quick-key-empty">{t("terminal.quickInput.noMatches")}</p>
